...
def make_flatten_left_join(arity)
  ...
  flat_manifests = TYPES[0..(arity - 1)].map { |x| "mf#{x}: ClassTag[#{x}]"}.join(", ")
  ...
  if arity <= MAX_IMPLICIT_ENRICHMENT_ARITY
    puts "#{$indent}class FlattenLeftJoin#{arity}[KEY, KLL[KLL_K, +KLL_V] <: KeyedListLike[KLL_K, KLL_V, KLL], #{flat_type}](nested: KLL[KEY, #{nested_type}])(implicit #{flat_manifests}) {"
    puts "#{$indent*2}def flattenValueTuple: KLL[KEY, (#{flat_type})] = nested.mapValues { tup => FlattenGroup.flattenNestedTuple(tup) }"
    puts "#{$indent}}"
    puts
    puts "#{$indent}implicit def toFlattenLeftJoin#{arity}[KEY, KLL[KLL_K, +KLL_V] <: KeyedListLike[KLL_K, KLL_V, KLL], #{flat_type}](nested: KLL[KEY, #{nested_type}])(implicit #{flat_manifests}) = new FlattenLeftJoin#{arity}(nested)"    
  end
...
def make_flatten_outer_join(arity)
  ...
  if arity <= MAX_IMPLICIT_ENRICHMENT_ARITY
    puts "#{$indent}class FlattenOuterJoin#{arity}[KEY, KLL[KLL_K, +KLL_V] <: KeyedListLike[KLL_K, KLL_V, KLL], #{flat_type}](nested: KLL[KEY, #{nested_type}])(implicit #{flat_manifests}) {"
    ...
    puts "#{$indent}}"
    puts
    puts "#{$indent}implicit def toFlattenOuterJoin#{arity}[KEY, KLL[KLL_K, +KLL_V] <: KeyedListLike[KLL_K, KLL_V, KLL], #{flat_type}](nested: KLL[KEY, #{nested_type}])(implicit #{flat_manifests}) = new FlattenOuterJoin#{arity}(nested)"
  end
...
puts %q|import scala.reflect.ClassTag
